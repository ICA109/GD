...
/*    public static int[] indices(int[] input) {
        ...
        System.out.println();
        System.out.println("Org. array: " + Arrays.toString(input));
        ...
        for(int i=0; i<input.length-1; i++) {
            if (input[i+1] > input[i]) {
                ...
                allSeq.add(actuallyAdded);
                possibleSeq.clear();    //Clear contents for next possible sequence (but turns out we're working with a reference)
                //System.out.println("Poss (pc): " + possibleSeq.toString());
                //System.out.println("actAdd (pc): " + actuallyAdded.toString());
                hasPossSubSeq = false;
            }
        }
        System.out.println("Midpoint Check: " + allSeq.toString());
    ...
    /*public static int[] indices(int[] input) {
        ...
        for(int i=0; i<input.length-1; i++) {
            if (input[i+1] > input[i]) {
                ...
                possibleSeq.add(input[i]);
                //possibleSeq.add(input[i+1]);
                //System.out.println("Poss: " + possibleSeq.toString());
                //i++;

                //Just add in special if condition where if last one is also increasing, terminate?
                if (i == input.length-2) {
                    possibleSeq.add(input[i+1]);
                    allSeq.add(possibleSeq);
                }
                ...
                if (hasPossSubSeq) {
                    possibleSeq.add(input[i]);  //We need to also include the final element despite fact i+1 is now decreasing
                }
                ...
                System.out.println("Poss (pc): " + possibleSeq.toString());
                System.out.println("actAdd (pc): " + actuallyAdded.toString());
                //i.e. clearing this means it's cleared in allSeq as well

                hasPossSubSeq = false;
            }
        }

        System.out.println("Final: " + allSeq.toString());
        return input;
    }*/
}
